...
class GaussD:
    ...
    def prob(self, observed_data):
        from scipy.stats import multivariate_normal
        T = observed_data.shape[1]
        pX = np.zeros(T)
        for t in range(T):
            pX[t] = multivariate_normal.pdf(observed_data[:,t], mean=self.means, cov=self.cov)
        
        return pX
    
    def logprob(self, observed_data):
        pX = self.prob(observed_data)
        log_pX = np.log(pX)
        return log_pX
