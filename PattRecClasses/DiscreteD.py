...
class DiscreteD:
    ...
    def rand(self, nData):
        ...
        output_set = np.arange(len(self.probMass))
        R = np.random.choice(a = output_set,size = nData,replace = True, p=self.probMass)
        return R
    ...
    def init(self, x):
        ...
        x = np.round(x)
        maxObs = int(np.max(x)+1)    #bug for translating matlab to python, python start from 0
        
        # collect observation frequencies
        fObs = np.zeros(maxObs) # observation frequencies
        
        for i in range(maxObs):
            fObs[i] = 1 + np.sum(x==i) #!here has one bug encountering the requirement, originally is x==i, if start from 1 then need i+1
